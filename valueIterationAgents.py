...
class ValueIterationAgent(ValueEstimationAgent):
        ...
        for _ in range(self.iterations):
            ...
            for state in self.mdp.getStates():
                ...
                for action in availableActions:
                    ...
                    qValue = sum([tsp[1]*(self.mdp.getReward(state, action, tsp[0])+self.discount*valuesCopy[tsp[0]]) for tsp in transitionStatesAndProbs])
                    if qValue > maxQValue:
                        maxQValue = qValue
    ...
    def computeQValueFromValues(self, state, action):
        ...
        "*** YOUR CODE HERE ***"
        transitionStatesAndProbs = self.mdp.getTransitionStatesAndProbs(state, action)
        return sum([tsp[1]*(self.mdp.getReward(state, action, tsp[0])+ self.discount * self.values[tsp[0]]) for tsp in transitionStatesAndProbs])
            ...
            for s in self.mdp.getStates():
                for action in self.mdp.getPossibleActions(s):
                    ...
                    for sp in transitions:
                        if state == sp[0] and sp[1] > 0:
                            predecessors[state].add(s)
