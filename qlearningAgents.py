...
class QLearningAgent(ReinforcementAgent):
    ...
    def computeValueFromQValues(self, state):
        ...
        if len(availableActions) == 0:
          return 0.0
        return max([self.getQValue(state, action) for action in availableActions])
    ...
    def update(self, state, action, nextState, reward):
        ...
        if (len(nextActions) == 0):
          ...
          sample = reward + self.discount * max([self.getQValue(nextState, action) for action in nextActions])
